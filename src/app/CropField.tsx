...
function CropField({ rows = 16, cols = 60 }) {
  const [now, setNow] = useState(0);
  
  useFrame(({clock}) => setNow(clock.getElapsedTime()));
  const flowers = [];
  for (let row = 0; row < rows; row++) {
    for (let col = 0; col < cols; col++) {
      ...
      flowers.push(
        <Sunflower key={`flower-${row}-${col}`} position={pos} windOffset={row * 0.6 + col * 0.4} windActive={cropWind} />
      );
    }
  }
  return (
